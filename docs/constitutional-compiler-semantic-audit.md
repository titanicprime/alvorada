...
| Foundational authority used derived-authority provenance fields. | Circular or fictional upstream authority could be manufactured, while an unsupported claim could be labeled documented. | Foundational human authority claims and derived grants now use distinct authority bases. Documented foundational provenance requires an actual evidence or artifact reference. Unknown provenance may have no evidence, while incomplete and disputed provenance may preserve partial or contested evidence without an invented source. Derived grants require an upstream source and prohibit self-attestation. | Documented provenance with and without evidence; unknown provenance without evidence; incomplete and disputed provenance; valid derivation; fictional source; and self-attestation cases. | The repository's named human authority remains a recorded claim whose external provenance may be unknown. |
| Governance lineage checks covered only generic rules and treated every dependency as governance lineage. | Governance objects could cite missing predecessors, while valid references to external artifacts, technical standards, datasets, or processes could be rejected. | Supersession, replacement, and explicitly governance-scoped dependency references are checked across supported governance objects. Generic dependencies remain representable without local governance-object resolution. | Broken governance references in each supported object collection, resolved cross-object lineage, and unresolved generic dependencies. | Historical and external artifacts are not fetched or factually verified; only explicitly governance-scoped lineage claims are resolved locally. |
