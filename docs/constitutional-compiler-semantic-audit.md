...
| Foundational authority used derived-authority provenance fields. | Circular or fictional upstream authority could be manufactured. | Foundational human authority claims and derived grants now use distinct authority bases. Foundational claims preserve evidenced, unknown, incomplete, or disputed provenance without an invented source; derived grants require an upstream source and prohibit self-attestation. | Foundational unknown provenance, valid derivation, fictional source, and self-attestation cases. | The repository's named human authority remains a recorded claim whose external provenance may be unknown. |
| Lineage checks covered only generic rules. | Authorities, delegations, offices, or events could cite missing predecessors or dependencies. | Supersession, replacement, and dependency references are checked across supported governance objects. | Broken references in each supported object collection and valid generic lineage. | Historical artifacts are not required to be migrated; only lineage claims made in validated records are checked. |
